...
if ($q !== '') {
    ...
    if (driver() === 'mysql') $where = str_replace('ILIKE','LIKE',$where);
    $params = ["%$q%","%$q%","%$q%"];
}

$total = (int)db()->prepare("SELECT COUNT(*) AS c FROM subdomains $where")->execute($params) ?: 0;
$st = db()->prepare("SELECT COUNT(*) AS c FROM subdomains $where");
$st->execute($params); $total = (int)$st->fetch()['c'];
...
<?php endforeach; ?>
<?php if (!$rows): ?><tr><td colspan="6" class="empty">Nenhum registro</td></tr><?php endif; ?>
</tbody></table>
